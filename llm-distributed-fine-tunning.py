...
if __name__ == "__main__":
    ...
    if not args.no_deepspeed:
        with open("finetune-gpt2-ray-test/deepspeed.json","r") as fp:
    ...
    data_set_path = download_file_from_s3(bucket_name, s3_file_name, local_file_name)
    #dataset = prepare_dataset(f"../data/{args.data}", args.model)
    
    dataset = prepare_dataset(data_set_path, args.model)
